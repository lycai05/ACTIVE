		...
		>
			<div class="search-box" @keydown.up="prevItem()" @keydown.down="nextItem()">
				<div class="search-input flex items-center">
					...
					<input placeholder="Search" v-model="search" class="grow" @input="fetchData" />
...
const fetchData = ()=> {
    axios.get('http://47.107.91.5/autocomplete',{
		params: {query: search.value}
	})
        .then(response => {
			console.log(response)
          filteredGroups.value = response.data.suggestions;
        })
        .catch(error => {
