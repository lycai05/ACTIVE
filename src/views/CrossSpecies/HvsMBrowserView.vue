<template>
	<CardWrapper v-slot="{ expand, isExpand, reload }" class="h-full grow w-full mb-6">
		<CardActions
			...
			:reload="reload"
			class="h-full"
			title="Browse genes or CREs with sequence homology"
			:segmented="{
				content: true,
				footer: true
			}"
		>
			<template #header>
				<div class="flex items-center justify-between w-full">
					...
					>
						<svg
							class="text-gray-500 dark:text-gray-300 group-hover:text-gray-900 dark:group-hover:text-white"
			...
			<template #default>
				<ExpressoVue
					:key="HumanBrowserKey"
					id="cross_species_human"
					:assembly="asm"
					:tracksInfo="tracksInfo"
					:highlight-region="highlightRegionHuman"
					:browser-id="'human'"
				...
				<ExpressoVue
					:key="MonkeyBrowserKey"
					id="cross_species_monkey"
					:assembly="asmMonkey"
					:highlight-region="highlightRegionMonkey"
					:tracksInfo="tracksInfoMonkey"
					:browser-id="'monkey'"
					@update:location="handleMonkeyLocationUpdate"
					:reverseLayout="asmMonkey.reverseLayout"
				></ExpressoVue>
			</template>
		</CardActions>
	</CardWrapper>
...
async function loadHumanBrowser() {
	HumanBrowser.value = defineAsyncComponent(() => import("@/browser/elements/Expresso/ExpressoVue.vue"))
}

async function loadMonkeyBrowser() {
	MonkeyBrowser.value = defineAsyncComponent(() => import("@/browser/elements/Expresso/ExpressoVue.vue"))
}
