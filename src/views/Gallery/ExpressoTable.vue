      ...
      const buttons = isHiC
        ? [
            { 
              ...
              action: async () => {
                const url = `https://3dgenomehub.oss-cn-shenzhen.aliyuncs.com/hic/${row.id}/${row.id}.compartments_E1.bigwig`;
            ...
            { 
              ...
              action: async () => {
                const url = `https://3dgenomehub.oss-cn-shenzhen.aliyuncs.com/hic/${row.id}/${row.id}.contact_domains.bed.gz`;
                const exists = await checkFileExists(url);
                if (exists) {
                  const trackData = {
            ...
            { 
              ...
              action: async () => {
                const url = `https://3dgenomehub.oss-cn-shenzhen.aliyuncs.com/hic/${row.id}/${row.id}.chromatin_loops.bed.gz`;
                const exists = await checkFileExists(url);
                if (exists) {
                  const trackData = {
            ...
            { 
              ...
              fileName: 'matrix.gz',
              action: async () => {
                const url = `https://3dgenomehub.oss-cn-shenzhen.aliyuncs.com/hic/${row.id}/${row.id}.matrix.gz`;
                const exists = await checkFileExists(url);
                if (exists) {
                  const trackData = {
        ...
        : [
            ...
            { 
              ...
              action: async () => {
                const url = `https://3dgenomehub.oss-cn-shenzhen.aliyuncs.com/hic/${row.id}/${row.id}.peaks.bed.gz`;
                const exists = await checkFileExists(url);
                if (exists) {
                  const trackData = {
...
const fnGetMenudata = () => {
  axios.get('http://47.107.91.5:8888/api/genomehub/humansamples/', {
    responseType: 'json',
  })
    .then(res => {
