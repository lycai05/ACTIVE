  ...
  <div ref="trackList" v-if="isLoaded" :id="props.sessionId" class="track-list bg-white flex flex-col p-2 w-full">
    ...
    <template v-if="!props.reverseLayout">
      <div class="track-item relative track-item-sortable flex flex-col mb-1" ref="trackItem" :id="`${props.trackId}`">
        <div class="flex">
          <div ref="cvs_holder" class="relative track-content flex-auto mr-2" style="min-height: 10px;min-width: 0;overflow: hidden">
            <nav-bar-track 
          ...
          <div class="track-item-config-holder track-item-drag-handler cursor-pointer bg-white rounded-r-lg relative w-6 flex-none flex flex-col justify-between items-center pt-1 pb-1">
          </div>
        </div>
      </div>
...
const handleZoomTo = (chrom: string, start: number, end: number, activeTrackViewIndex: number | null) => {
  ...
  navigationStore.zoomTo({ chrom, start, end }, activeTrackViewIndex !== null ? activeTrackViewIndex : undefined, minRegionSize);
};
...
const handleShift = (ev: any, direction: number, activeTrackViewIndex: number | null) => {
  ...
  navigationStore.handleShift(ev, direction, activeTrackViewIndex !== null ? activeTrackViewIndex : undefined, minRegionSize);
};
...
// Watchers
watch(() => navigationStore.$state, (newValue) => {
