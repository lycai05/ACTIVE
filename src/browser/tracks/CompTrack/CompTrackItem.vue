...
class DataCache {
	...
	findOverlappingData(chrom: string, start: number, end: number): Feature[] | null {
		for (const [key, features] of this.cache.entries()) {
			...
			if (cachedChrom === chrom && this.hasOverlap([start, end], [cachedStart, cachedEnd])) {
				return features
			}
		}
		return null
...
function updateChart(featuresData: Feature[]) {
	...
	if (props.option.yAxis?.log) {
		data = data.map(item => ({
			...item,
			y: item.y === 0 ? 0 : item.y > 0 ? Math.log10(item.y) : -Math.log10(Math.abs(item.y))
		}))
	}
...
<template>
	<div class="relative h-full w-full" :style="props.style">
		...
		<div ref="canvasContainer" class="basic-canvas"></div>
	</div>
</template>
