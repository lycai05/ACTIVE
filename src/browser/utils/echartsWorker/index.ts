...
function getEventBinders(worker: Worker) {
	...
	worker.addEventListener('message', (event: MessageEvent<{ type: string; data: unknown }>) => {
		const scope = 'echarts:'
 ...
 * Show loading.
 */
function showLoading(this: Worker, ...args: Parameters<ECharts['showLoading']>) {
	this.postMessage({
		type: 'showLoading',
		args
	})
 ...
 * Hide loading.
 */
function hideLoading(this: Worker, ...args: Parameters<ECharts['hideLoading']>) {
	this.postMessage({
		type: 'hideLoading',
		args
	})
...
function init(canvas: HTMLCanvasElement, theme: Theme = undefined, option: InitOption = {}) {
